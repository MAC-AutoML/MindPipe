...
def _install_compat_shims() -> None:
    ...
    if not hasattr(transformers, "Gemma3Config"):
        _set_transformers_attr(
            transformers, "Gemma3Config", transformers.PretrainedConfig
        )
        ...
        class _AutoVideoProcessor:
            ...
            def from_pretrained(cls, *args, **kwargs):
                raise NotImplementedError(
                    "AutoVideoProcessor shim is not available in this transformers build."
                )
    ...
    if "vllm.transformers_utils.gguf_utils" not in sys.modules:
        ...
        gguf_utils.maybe_patch_hf_config_from_gguf = (
            lambda config, *args, **kwargs: config
        )
...
def main() -> int:
    ...
    print(
        json.dumps(
            {"prompt": args.prompt, "completion": text}, ensure_ascii=False, indent=2
        )
    )
    return 0
