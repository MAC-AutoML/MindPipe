...
# User-facing names are mapped to VLMEvalKit canonical names below.
...
MPL_CONFIG_DIR="${MPL_CONFIG_DIR:-/tmp/mpl_qwen2_5_vl_gptq_vlm_gpu${GPU_ID}}"
SKIP_EXISTING="${SKIP_EXISTING:-true}"
DRY_RUN="${DRY_RUN:-false}"
SMOKE="${SMOKE:-false}"
OUTPUT_ROOT="${OUTPUT_ROOT:-$REPO_ROOT/new_results/quantization_suite/gptq_qwen2_5_vl_7b_chartqa100_vlm}"
...
if [[ -z "${MODEL_PATH:-}" || ! -d "$MODEL_PATH" ]]; then
  echo "[ERROR] Qwen2.5-VL model path not found."
  echo "Set MODEL_PATH manually, e.g.:"
  echo "  MODEL_PATH=/path/to/Qwen2.5-VL-7B-Instruct"
  exit 1
fi
...
required = ("torch", "transformers", "qwen_vl_utils")
...
if device.startswith("cuda") and not torch.cuda.is_available():
    ...
    sys.exit(2)
print("[INFO] dependency check passed: torch/transformers/qwen_vl_utils")
PY
fi
