...
OUTPUT_ROOT="${OUTPUT_ROOT:-$REPO_ROOT/new_results/quantization_suite/gptq_qwen3_5_4b}"
...
if [[ -z "${MODEL_PATH:-}" || ! -d "$MODEL_PATH" ]]; then
  echo "[ERROR] MODEL_PATH not found for Qwen3.5-4B: ${MODEL_PATH:-<empty>}"
  echo "Set MODEL_PATH manually, e.g.:"
  echo "  MODEL_PATH=/path/to/Qwen3.5-4B"
  exit 1
fi
