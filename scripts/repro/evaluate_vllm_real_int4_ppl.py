...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--model", required=True, help="Self-contained vLLM model export directory.")
    parser.add_argument("--dataset", default="wikitext2", choices=["wikitext2", "c4"])
    parser.add_argument("--data_path", default="/mnt/42_store/lcw/data2/Huawei/datasets")
...
def _patch_vllm_int4_contiguous_inputs() -> None:
    ...
    if not getattr(ExllamaLinearKernel.apply_weights, "_mindpipe_contiguous_patch", False):
    ...
    if not getattr(MarlinLinearKernel.apply_weights, "_mindpipe_contiguous_patch", False):
...
def _token_logprob(token_id: int, prompt_logprob_entry) -> float:
    ...
    raise KeyError(f"Token id {token_id} not found in prompt logprobs; available keys: {available}")
