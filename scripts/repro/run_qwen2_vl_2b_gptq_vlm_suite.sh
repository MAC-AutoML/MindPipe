...
MPL_CONFIG_DIR="${MPL_CONFIG_DIR:-/tmp/mpl_qwen2_vl_2b_gptq_vlm_gpu${GPU_ID}}"
SKIP_EXISTING="${SKIP_EXISTING:-true}"
DRY_RUN="${DRY_RUN:-false}"
SMOKE="${SMOKE:-false}"
OUTPUT_ROOT="${OUTPUT_ROOT:-$REPO_ROOT/new_results/quantization_suite/gptq_qwen2_vl_2b_chartqa100_vlm}"
...
if [[ -z "${MODEL_PATH:-}" || ! -d "$MODEL_PATH" ]]; then
  echo "[ERROR] Qwen2-VL model path not found."
