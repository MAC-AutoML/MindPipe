...
OUTPUT_ROOT="${OUTPUT_ROOT:-$REPO_ROOT/new_results/quantization_suite/gptq_qwen3_0_6b}"
...
if [[ -z "${MODEL_PATH:-}" || ! -d "$MODEL_PATH" ]]; then
  echo "[ERROR] MODEL_PATH not found for Qwen3-0.6B: ${MODEL_PATH:-<empty>}"
  echo "Set MODEL_PATH manually, e.g.:"
  echo "  MODEL_PATH=/path/to/Qwen3-0.6B"
  exit 1
fi
