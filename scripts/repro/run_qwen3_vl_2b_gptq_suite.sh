...
OUTPUT_ROOT="${OUTPUT_ROOT:-$REPO_ROOT/new_results/quantization_suite/gptq_qwen3_vl_2b}"
...
if [[ -z "${MODEL_PATH:-}" || ! -d "$MODEL_PATH" ]]; then
  echo "[ERROR] MODEL_PATH not found for Qwen3-VL-2B: ${MODEL_PATH:-<empty>}"
  echo "Set MODEL_PATH manually, e.g.:"
  echo "  MODEL_PATH=/path/to/Qwen3-VL-2B"
  exit 1
fi
...
required = ("torch", "transformers", "lm_eval")
