...
def load_dequantized_export(model, export_dir: Path) -> dict[str, int]:
    ...
    for path in safetensors_files:
        with safe_open(path, framework="pt", device="cpu") as handle:
            packed_names = sorted(name for name in handle.keys() if name.endswith(".weight_packed"))
            for packed_name in packed_names:
                ...
                module.weight.data.copy_(
                    dequantized.to(device=module.weight.device, dtype=module.weight.dtype)
                )
                loaded += 1
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--zero_shot_tasks",
        nargs="+",
        default=["boolq", "rte", "winogrande", "arc_easy", "arc_challenge", "openbookqa"],
    )
