...
MODEL_PATH="${MODEL_PATH:-/mnt/82_store/LLM-weights/Qwen3-32B}"
PRUNING="${PRUNING:-wanda}"
GPU_ID="${GPU_ID:-1,2,3,4}"
...
SPARSITY_RATIO="${SPARSITY_RATIO:-0.52}"
...
COMPRESSION_LORA_TRAIN_PLAN="${COMPRESSION_LORA_TRAIN_PLAN:-sft}"
...
COMPRESSION_LORA_CPT_SAMPLES="${COMPRESSION_LORA_CPT_SAMPLES:-512}"
...
COMPRESSION_LORA_SFT_TRAIN_FILE="${COMPRESSION_LORA_SFT_TRAIN_FILE:-/mnt/42_store/wxx/datasets/flan_mini_filtered_seq512/flan_mini_filtered_seq512.json}"
COMPRESSION_LORA_SFT_SAMPLES="${COMPRESSION_LORA_SFT_SAMPLES:-1024}"
...
COMPRESSION_LORA_INIT="${COMPRESSION_LORA_INIT:-pissa}"
COMPRESSION_LORA_BATCH_SIZE="${COMPRESSION_LORA_BATCH_SIZE:-4}"
...
COMPRESSION_LORA_WEIGHT_CHECKPOINTING="${COMPRESSION_LORA_WEIGHT_CHECKPOINTING:-true}"
...
cmd=(
  ...
  # --flatquant_resume_from /mnt/82_store/wxx/HWQuant/Mindpipe/results_lora/Qwen3-32B/flatquant/flatquant_w3a16_q16k16v16_seq512
  ...
  --compression_lora_weight_checkpointing "$COMPRESSION_LORA_WEIGHT_CHECKPOINTING"
