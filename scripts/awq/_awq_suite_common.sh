...
if [[ -n "${MINDPIPE_AWQ_SUITE_COMMON_SH_LOADED:-}" ]]; then
  return 0
fi
MINDPIPE_AWQ_SUITE_COMMON_SH_LOADED=1

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
ALGORITHM="awq"

mindpipe_awq_init_defaults() {
  ...
  MODEL_DISPLAY_NAME="${MODEL_DISPLAY_NAME:-${MODEL_NAME:-model}}"
  MODEL_TAG="${MODEL_TAG:-model}"
  OUTPUT_ROOT="${OUTPUT_ROOT:-$REPO_ROOT/new_results/quantization_suite/awq_${MODEL_TAG}}"
  REQUIRED_MODULES_STR="${REQUIRED_MODULES_STR:-torch transformers lm_eval}"
}

mindpipe_awq_pick_model_path() {
  local -a candidates=("$@")
...
mindpipe_awq_append_optional_args() {
  local -n cmd_ref=$1
  if [[ -n "${HF_TOKEN:-}" ]]; then
...
mindpipe_awq_run_fp_baseline() {
  local run_tag="fp16_seq${FP_SEQUENCE_LENGTH}"
  local run_output="$OUTPUT_ROOT/$run_tag"
  local metrics_path
  metrics_path="$(mindpipe_awq_find_metrics_file "$run_output")"

  if [[ "$SKIP_EXISTING" == "true" && -n "$metrics_path" ]] && mindpipe_awq_is_metrics_complete "$metrics_path"; then
  ...
  mindpipe_awq_append_optional_args cmd
  mindpipe_awq_run_command "$log_path" "${cmd[@]}"
  RUN_TAGS+=("$run_tag")
}

mindpipe_awq_run_quant_config() {
  local weight_bits="$1"
  local sequence_length="$2"
  local run_tag="w${weight_bits}a16_g${WEIGHT_GROUP_SIZE}_seq${sequence_length}"
  local run_output="$OUTPUT_ROOT/$run_tag"
  local metrics_path
  metrics_path="$(mindpipe_awq_find_metrics_file "$run_output")"
  ...
  local -a cmd=(
    ...
    --awq_search_sequence_length "$sequence_length"
...
mindpipe_awq_summarize_results() {
...
mindpipe_awq_prepare_runtime_env() {
...
mindpipe_awq_check_dependencies() {
  if [[ "$DRY_RUN" == "true" ]]; then
    return 0
  fi
...
mindpipe_awq_validate_entry() {
  if [[ ! -x "$PYTHON_BIN" ]]; then
    echo "[ERROR] PYTHON_BIN is not executable: $PYTHON_BIN"
    exit 1
  fi
  if [[ -z "${MODEL_NAME:-}" ]]; then
    echo "[ERROR] MODEL_NAME must be set before calling mindpipe_awq_main"
    exit 1
  fi
  if [[ -z "${MODEL_PATH:-}" || ! -d "$MODEL_PATH" ]]; then
    echo "[ERROR] MODEL_PATH not found for $MODEL_DISPLAY_NAME: ${MODEL_PATH:-<empty>}"
    echo "Set MODEL_PATH manually, e.g.:"
    echo "  MODEL_PATH=/path/to/$MODEL_NAME"
    exit 1
  fi
}

mindpipe_awq_print_banner() {
  ...
  echo "[INFO] plan: fp16(seq=$FP_SEQUENCE_LENGTH) + awq(w{2,3,4}a16 g${WEIGHT_GROUP_SIZE} @ seq=512,2048) => 7 runs"
}
...
mindpipe_awq_main() {
  ...
  for sequence_length in "${SEQ_LENGTHS[@]}"; do
    for weight_bits in "${WEIGHT_BITS_LIST[@]}"; do
      mindpipe_awq_run_quant_config "$weight_bits" "$sequence_length"
    done
  done
