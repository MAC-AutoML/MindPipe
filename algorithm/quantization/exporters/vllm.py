    ...
    def __init__(
        ...
        *,
        ...
        self.register_buffer(
            "weight_scale", scale.detach().cpu().to(torch.float16).contiguous()
        )
        self.register_buffer(
            "weight_shape",
...
def pack_int4_for_vllm(int_weight: torch.Tensor) -> torch.Tensor:
    ...
    if int_weight.ndim != 2:
        raise ValueError(
            f"Expected 2D int_weight, got shape {tuple(int_weight.shape)}."
        )
    if int_weight.numel() and (int_weight.min() < -8 or int_weight.max() > 7):
        raise ValueError("int4 weights must be in signed range [-8, 7].")

    weight = (
        (int_weight.to(torch.int16) + 8).to(torch.uint8).cpu().numpy().astype(np.uint32)
    )
    pack_factor = 8
...
def _update_config_json(
    export_dir: Path, *, group_size: int, ignored_layers: list[str]
) -> None:
...
def export_vllm_gptq_w4a16(
    *,
    ...
    if not artifacts:
        raise ValueError(
            "No real-quant Linear artifacts were provided for vLLM export."
        )
    ...
    try:
        for name, artifact in artifacts.items():
            ...
            if not isinstance(module, nn.Linear):
                raise TypeError(
                    f"Module {name!r} is {type(module)!r}, expected nn.Linear."
                )
            if artifact.bits != 4 or not artifact.symmetric:
                raise ValueError(
                    f"Only symmetric W4 artifacts are supported, got {artifact!r}."
                )
            ...
            expected_scale_shape = (
                module.out_features,
                module.in_features // int(group_size),
            )
        ...
        copied_inference_files = _copy_inference_files_from_source(
            tokenizer_bundle, export_path
        )
        _update_config_json(
            export_path, group_size=group_size, ignored_layers=ignored_layers
        )
    finally:
