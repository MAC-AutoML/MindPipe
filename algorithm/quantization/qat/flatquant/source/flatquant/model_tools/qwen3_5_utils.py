...
class FlatQuantQwen3_5MoeAttention(FlatQuantQwen3_5Attention):
    ...
    def forward(
        ...
        **kwargs,
        ...
        del use_cache
        if position_embeddings is None:
            raise AttributeError("FlatQuantQwen3_5MoeAttention requires `position_embeddings` from the parent decoder layer.")
...
class FlatQuantQwen3_5MoeGatedDeltaNet(FlatQuantQwen3_5GatedDeltaNet):
    def forward(
        self,
        hidden_states: torch.Tensor,
        attention_mask: torch.Tensor | None = None,
...
def _apply_trans_to_weight(weight: torch.Tensor, trans: nn.Module) -> torch.Tensor:
    if isinstance(trans, list):
        raise NotImplementedError("MoE FlatQuant does not use list-based transforms.")
...
class FlatQuantQwen3_5MoeMLP(nn.Module):
    def __init__(self, args, module: nn.Module):
        super().__init__()
        self.args = args
        self.gate_proj = FlatQuantizedLinear(args, module.gate_proj)
...
class FlatQuantQwen3_5MoePackedExperts(nn.Module):
    def __init__(self, args, module: nn.Module):
        super().__init__()
        self.args = args
...
class FlatQuantQwen3_5MoeSparseMoeBlock(nn.Module):
    def __init__(self, args, module: nn.Module):
        super().__init__()
        self.gate = module.gate
        self.experts = FlatQuantQwen3_5MoePackedExperts(args, module.experts)
        self.shared_expert = FlatQuantQwen3_5MoeMLP(args, module.shared_expert)
    ...
    def add_fq_trans(self):
        ...
        if self.experts.args.w_bits < 16 or self.experts.args.a_bits < 16:
            self._moe_in_trans = _build_group_trans(self.experts.args, sample_weight.shape[1])
        else:
...
class FlatQuantQwen3_5MoeDecoderLayer(nn.Module):
    def __init__(self, args, ori_layer):
        super().__init__()
        self.hidden_size = ori_layer.hidden_size
        self.layer_type = ori_layer.layer_type
        if self.layer_type == "linear_attention":
            self.self_attn = FlatQuantQwen3_5MoeGatedDeltaNet(args, ori_layer.linear_attn)
        elif self.layer_type == "full_attention":
            self.self_attn = FlatQuantQwen3_5MoeAttention(args, ori_layer.self_attn)
        else:
            raise ValueError(f"Unsupported Qwen3.5-MoE layer_type: {self.layer_type}")
        self.mlp = FlatQuantQwen3_5MoeSparseMoeBlock(args, ori_layer.mlp)
...
def apply_flatquant_to_qwen3_5_moe(args, model):
    skip_initialization()
    decoder_root = _decoder_root(model)
    for layer_index in range(len(decoder_root.layers)):
        decoder_root.layers[layer_index] = FlatQuantQwen3_5MoeDecoderLayer(
            args,
            decoder_root.layers[layer_index],
        )
    return model
