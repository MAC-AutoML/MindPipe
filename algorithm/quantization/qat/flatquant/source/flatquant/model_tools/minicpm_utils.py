...
class FlatQuantMiniCPMMLP(nn.Module):
    ...
    def add_fq_trans(self):
        ...
        if self.args.w_bits < 16 or self.args.a_bits < 16:
            ...
            self.up_gate_trans = decompose_trans_matrix(up_dim_left, up_dim_right, add_diag=self.args.add_diag)
            down_dim_left, down_dim_right = get_decompose_dim(self.down_proj.linear.weight.shape[1])
            self.down_trans = decompose_trans_matrix(down_dim_left, down_dim_right, add_diag=self.args.add_diag)
        else:
...
class FlatQuantMiniCPMAttention(nn.Module):
    def __init__(self, args, module):
        super().__init__()
        self.args = args
        object.__setattr__(self, "_original_module", module)
    ...
    def add_fq_trans(self):
        ...
        if self.args.w_bits < 16 or self.args.a_bits < 16:
            ...
            self.ln_trans = decompose_trans_matrix(ln_dim_left, ln_dim_right, add_diag=self.args.add_diag)
...
def apply_flatquant_to_minicpm(args, model):
    skip_initialization()
    for layer_index in tqdm(range(model.config.num_hidden_layers), desc="Applying FlatQuant to model"):
        model.model.layers[layer_index].self_attn = FlatQuantMiniCPMAttention(
            args,
            model.model.layers[layer_index].self_attn,
        )
        model.model.layers[layer_index].mlp = FlatQuantMiniCPMMLP(
            args,
