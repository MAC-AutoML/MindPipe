...
from splitquant.function_utils import set_require_grad_all, get_n_set_parameters_byname, get_paras_dict_by_name, check_params_grad
    ...
    for attr_name in ("linear_u_list", "linear_v_list"):
        if hasattr(module, attr_name):
            for linear in getattr(module, attr_name):
                _reset_square_linear_to_identity(linear)
    if hasattr(module, "linear_diag_list"):
        for linear_diag in module.linear_diag_list:
            linear_diag.data.fill_(1)
    for attr_name in (
        "linear",
        "linear_left",
        "linear_right",
...
def _stabilize_splitquant_layer(layer):
...
def cali_split_quant(args, model, dataloader, dev, logger):
    model.eval()
    ...
    for i in range(num_train_layer):
        ...
        if batched_layer_kwargs_by_type is not None:
            batched_layer_kwargs_by_type = {
                lt: move_tensors_to_device(kw, layer_dev)
                for lt, kw in batched_layer_kwargs_by_type.items()
            }
                ...
                for j in range(args.nsamples // args.cali_bsz):
                    ...
                    try:
                        ...
                        if "singular" in error_text or "linalg" in error_text or "inverse" in error_text:
                            ...
                            _stabilize_splitquant_layer(layer)
                            unstable_layer = True
                            break
                        raise
