...
class ActivationQuantizer(torch.nn.Module):
    '''
        A class for quantizing the activations. We support per-token quantization and grouped per-token
        quantization along the last dimension.
    '''
    def __init__(self, bits, sym=False, lac=False, groupsize=-1, clip_ratio=None, in_channels=-1):
        ...
        if self.lac:
            ...
            if self.groupsize > 0:
                assert in_channels > 0 and in_channels % self.groupsize == 0
                clip_shape = (in_channels // self.groupsize,)
            else:
                clip_shape = (1,)
            self.clip_factor_a_max = torch.nn.Parameter(torch.ones(clip_shape) * init_value, requires_grad=True)
            self.clip_factor_a_min = torch.nn.Parameter(torch.ones(clip_shape) * init_value, requires_grad=True)
    ...
    def get_scale_zero(self, x):
        ...
        if self.groupsize <= 0:
            ...
            last_dim = x.shape[-1]
            assert last_dim % self.groupsize == 0
            x_reshaped = x.reshape(-1, last_dim // self.groupsize, self.groupsize)
            xmax = x_reshaped.amax(dim=-1, keepdim=True)
            xmin = x_reshaped.amin(dim=-1, keepdim=True)
            tmp = torch.zeros_like(xmax)
            xmax = torch.maximum(xmax, tmp)
            xmin = torch.minimum(xmin, tmp)
            if self.lac:
                clip_factor_a_max = self.sigmoid(self.clip_factor_a_max).view(1, -1, 1)
                clip_factor_a_min = self.sigmoid(self.clip_factor_a_min).view(1, -1, 1)
            ...
            if self.sym:
                ...
                scale = xmax / q_max
                scale[tmp] = 1
                scale = scale.expand(-1, -1, self.groupsize).reshape(init_shape)
                zero = torch.zeros_like(scale)
            else:
                tmp = (xmin == 0) & (xmax == 0)
                xmin = xmin.clone()
                ...
                zero = torch.round(-xmin / scale)
                scale = scale.expand(-1, -1, self.groupsize).reshape(init_shape)
                zero = zero.expand(-1, -1, self.groupsize).reshape(init_shape)
