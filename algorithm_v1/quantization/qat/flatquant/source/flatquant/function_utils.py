...
def get_inverse(matrix):
    ...
    try:
        return matrix64.inverse().to(dtype)
    except RuntimeError:
        eye = torch.eye(matrix64.shape[-1], device=matrix64.device, dtype=matrix64.dtype)
        regularization = 1e-6
        for _ in range(6):
            try:
                return torch.linalg.inv(matrix64 + eye * regularization).to(dtype)
            except RuntimeError:
                regularization *= 10
        return torch.linalg.pinv(matrix64).to(dtype)
