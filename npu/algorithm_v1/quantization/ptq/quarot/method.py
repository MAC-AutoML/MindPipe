...
from ....common.device import preferred_rotation_dtype
...
class QuaRotMethod(BaseQuantizationMethod):
    ...
    def _apply_runtime_rotation(self, model, backbone, args, quarot_runtime) -> None:
        ...
        embed.weight.data = torch.matmul(embed.weight.data.to(rotation_dtype), q_matrix.to(rotation_dtype)).to(
            embed.weight.dtype
        )
        ...
        if head is not None:
            ...
            head.weight.data = torch.matmul(head.weight.data.to(rotation_dtype), q_matrix.to(rotation_dtype)).to(
                head.weight.dtype
            )
            if hasattr(model, "lm_head"):
        ...
        for module in self._get_extra_rotation_modules(model):
            ...
            module.weight.data = torch.matmul(q_matrix.T.to(rotation_dtype), module.weight.data.to(rotation_dtype)).to(
                module.weight.dtype
            )
            if module.bias is not None:
                module.bias.data = torch.matmul(q_matrix.T.to(rotation_dtype), module.bias.data.to(rotation_dtype)).to(
                    module.bias.dtype
                )
            module.cpu()
        ...
        for layer_index, block in enumerate(backbone.layers):
            ...
            for linear in (block.self_attn.q_proj, block.self_attn.k_proj, block.self_attn.v_proj):
                linear.weight.data = torch.matmul(
                    linear.weight.data.to(rotation_dtype),
                    q_matrix.to(rotation_dtype),
                ).to(linear.weight.dtype)

            block.self_attn.o_proj.weight.data = torch.matmul(
                q_matrix.T.to(rotation_dtype),
                block.self_attn.o_proj.weight.data.to(rotation_dtype),
            ...
            if block.self_attn.o_proj.bias is not None:
                block.self_attn.o_proj.bias.data = torch.matmul(
                    q_matrix.T.to(rotation_dtype),
                    block.self_attn.o_proj.bias.data.to(rotation_dtype),
